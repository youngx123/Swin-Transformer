...
def segmentation_loss(y_pred, y_true, buffer_step, AnnealingSoftmaxFocalloss=False):
        # if softmax_focalloss:
        #     return softmax_focalloss(y_pred, y_true.long(), ignore_index=255,
        #                              gamma=self.config.softmax_focalloss.gamma,
        #                              normalize=self.config.softmax_focalloss.normalize)
        if AnnealingSoftmaxFocalloss:
            func_dict = dict(cosine=cosine_annealing, poly=poly_annealing, linear=linear_annealing)

            return annealing_softmax_focalloss(y_pred, y_true.long(), buffer_step, 10000,
                                               255, 2, func_dict['cosine'])

        return F.cross_entropy(y_pred, y_true.long(), ignore_index=255)
